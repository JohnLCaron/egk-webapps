...
fun Route.serverRouting() {
    val encryption = EncryptionService.getInstance()
    ...
    route("/egk") {
        ...
        // authenticate("auth-basic") {
            ...
            post("encryptBallot/{device}") {
                ...
                val result = encryption.encrypt(device, plaintextBallot)
                if (result is Ok) {
                    ...
                    //call.respondText(
                    //    "EgkServer encrypt ballot id=${plaintextBallot.ballotId} success",
            ...
            get("castBallot/{device}/{ccode}") {
                ...
                val result = encryption.submit(device, ccode, EncryptedBallot.BallotState.CAST)
                if (result is Ok) {
                    call.respondText(
            ...
            get("spoilBallot/{device}/{ccode}") {
                ...
                val result = encryption.submit(device, ccode, EncryptedBallot.BallotState.SPOILED)
                if (result is Ok) {
                    call.respondText(
